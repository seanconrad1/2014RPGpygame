...
def help():
	print ("COMMANDS: pstats (Player's stats), wstats (Weapon stats), gold, i (Inventory), fight, market, sw weapons (Switch Weapons)")
...
def combat(weapon, enemy):
	...
	while execute == True:
		...
		if player['health'] <= 0:
			...
			gold = gold + enemy['gold']
			slowText("You receive " + str(enemy['gold']) + " gold!")
			#enemy['health'] = 10 <------ TEST ME OUT, RESETS ENEMY HEALTH
			break
